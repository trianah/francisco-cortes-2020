...
const maxSizes = (from, to, slices) => {
  const values = numRange(
    from,
    to * slices,
    counter => (counter / slices) * 100
  );
...
function negativeSpacing(num = 12, slices = 4) {
  const rows = -1 * num * slices;
  const values = numRange(rows, 0, counter => counter / slices);
